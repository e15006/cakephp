...
class TunesController extends AppController
{
    ...
    function search()
    {
        ...
        $fopts = $this->Tunes->Feelings->find('list')->toArray();
        $fopts['0'] = '指定しない';
        ksort($fopts);
        ...
        $query = $query->where(['Tunes.artist_id' => $this->request->data['artist_id']]);            if(!empty($this->request->data['Feelings_id'])){
                debug($this->request->data['Feelings_id']);
        $query = $query->where(['Tunes.Feelings_id' => $this->request->data[Feelings_id]]);
            }
        }
